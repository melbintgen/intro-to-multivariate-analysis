...
**2.1 Loading plots:** each bar represent the contribution (= coefficient) of each gene in each component. Here we only show the top 50 genes with the highest contribution (positive or negative sign, going from the bottom of the graph to the top). The argument `contrib = 'max'` colors the bars according to the tumour subtype for which the mean expression value is maximal. The argument  `contrib = 'min'` colors the bars according to the tumour subtype for which the mean expression value is minimal. When we change to  the argument `comp = 2`, we observe a new set of genes and the color patterns are different. 
...
- https://mixomics.org/mixdiablo/diablo-tcga-case-study/ (initial analysis part)
