...
We run similar command lines, but this time we use *sparse PLS-DA* that will enable us to select the top discriminant variables. The variable selection process is done within PLS-DA. For this, we need to specify the number of variables to select per component.
...
*note: sorry, we have a bug with the biplot that we need to fix!
